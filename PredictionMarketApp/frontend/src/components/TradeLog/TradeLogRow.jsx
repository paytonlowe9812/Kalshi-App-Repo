  ...
  return (
    <div className={`border-b border-terminal-border-dim/30 ${pnl > 0 ? 'bg-terminal-green/5' : pnl < 0 ? 'bg-terminal-red/5' : ''}`}>
      <div className="hidden md:flex items-center gap-3 px-4 py-2 text-xs font-mono">
        <span className="text-terminal-amber-dim w-16">{trade.logged_at ? new Date(trade.logged_at.replace(' ', 'T')).toLocaleTimeString() : '--'}</span>
      ...
      <div className="md:hidden flex items-center gap-3 px-3 py-2.5 active:bg-terminal-amber-faint/50">
        ...
        <div className="flex flex-col items-end flex-shrink-0"><span className={`text-xs font-mono font-semibold ${pnl > 0 ? 'text-terminal-green-text' : pnl < 0 ? 'text-terminal-red-text' : 'text-terminal-amber-dim'}`}>{pnl !== 0 ? `$${pnl.toFixed(2)}` : '--'}</span><span className="text-[10px] text-terminal-amber-muted font-mono">{trade.logged_at ? new Date(trade.logged_at.replace(' ', 'T')).toLocaleTimeString() : ''}</span></div>
